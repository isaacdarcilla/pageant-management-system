...
$contest_id = $_GET['contest_id'];
 ...
 $querSponsorName = "SELECT sponsor_name 
    ...
    $querConName = "SELECT contest_name 
                        FROM contest 
                            WHERE contest_id = $contest_id";
    $resConName = mysqli_query($connection, $querConName);
    $rowConName = mysqli_fetch_assoc($resConName);    

    if(isset($_POST['yes']))
    {
      $string = "00-auto-";
      $contest_name = $_POST['new_contest'];
            $query = "SELECT contest_name 
                        FROM contest 
                            WHERE contest_id=$contest_id";
            $result = mysqli_query($connection, $query);
            $row = mysqli_fetch_assoc($result);
            $contest_name_old = $string.str_replace(' ', '', strtolower($row['contest_name']));
            ...
            $query = "ALTER TABLE $contest_name_old 
                        RENAME TO $contest_name";
            $result = mysqli_query($connection, $query);


              $event_args = $row['contest_name']. ' successfully renamed to '.$_POST['new_contest'];
              $status = 'Rename';
...
<div class="padding">
  ...
  <form id="custom2" name="form" class="form-group column" action="" ui-jp="parsley" method="post"><br><center><div style="font-size:  75px; padding-top: 75px;" ><span class="fa  fa-edit text-success"></span></div>
    <h2 class="" style="font-style: 50px; padding-top: 0px;"><span class="text-warning">Do you want to edit <strong><?php echo $rowContest['contest_name']?></strong>?</center></span></h2><br>
      ...
      <div class="col-sm-10">
        <div class="row">
          <div class="col-md-8" style="padding-left: 100px;">
            <input type="text" name="new_contest" class="form-control rounded"  placeholder="Rename <?php echo $rowContest['contest_name']?>" required>
          </div>
       </div><br>
        <div class="row">
          ...
          <div class="col-md-2">
         <button type="submit"  name="yes" class="btn rounded white btn-block">Edit</button>
       </div>
