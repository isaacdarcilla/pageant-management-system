...
$username = $_GET['username'];
  ...
  $time = '';
    ...
    {
      header("location:admin_register_organizer.php?user_id=$user_id&username=$username");
    } 
    ...
    $password = $_POST["password"];
      $encrypt  = md5($password);
        ...
        if(mysqli_affected_rows($connection)>0)
        {
            ...
            $message = "Organizer name already exists. Try a new name.";
        }
        else
        {
             $numberOfEntity = 0;
              $queryEventAll = "SELECT event_name FROM event_table WHERE created_by=$user_id";
                $resultEventAll = mysqli_query($connection, $queryEventAll);

                $rowEventAll = mysqli_fetch_assoc($resultEventAll);
                
                $event_res = $rowEventAll['event_name'];
            
            $query_data = "INSERT INTO sponsor_organizer  VALUES ('NULL','$sponsor_name', '$in_charge', '$position', 
              '$established', '$email', '$phone_number', '$username1', '$encrypt', '$user_id','$password');"; 
            $insert = mysqli_query($connection, $query_data)
            


      or die("Could not insert data because $query_data".mysqli_connect_error());
      ...
      // print a success message
        header("location:admin_organizer.php?user_id=$user_id&username=$username");
...
<!-- ############ LAYOUT START--><br>
<center><h2 class=" _700 l-s-n-1x m-b-md">Register an<span class="text-primary"> Organizer</span></h2></center>
 ...
 <form id="custom2" name="form" class="form-group column" action="" method="post">
   <div class="form-group row">
      ...
      <div class="col-sm-10">
        <div class="row">
          <div class="col-md-5">
            <input type="text" name="sponsor_name" class="form-control rounded" placeholder="Organization Name" required>
          </div>
          <div class="col-md-4">
            <input type="text" name="in_charge" class="form-control rounded" placeholder="Who is in charge?" required>
          </div>
        </div>
      </div>
    </div>
   <div class="form-group row">
      ...
      <div class="col-sm-10">
        <div class="row">
          <div class="col-md-4">
            <input type="text" name="position" class="form-control rounded" placeholder="Position" required>
          </div>
  <div class="col-md-5">
          <div class='input-group date' ui-jp="datetimepicker" ui-options="{
              ...
              <input type='text' name="established" required class="form-control rounded" placeholder="Date Established"/>
   ...
   <div class="form-group row">
      ...
      <div class="col-sm-10">
        <div class="row">
          <div class="col-md-4">
            <input type="text" name="username_o" class="form-control rounded" placeholder="Username" required>
          </div>
      <div class="col-md-3">
            <input type="password" name="password" value="" class="form-control rounded" placeholder="Password" required>
          </div>

          <div class="col-md-2">
         <button type="submit"  name="add_organizer" class="btn rounded primary btn-block">Register</button>
       </div>
        </div>
      </div>
    </div>
