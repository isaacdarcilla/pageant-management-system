...
<?php
        ...
        $tabulatedQuery = "SELECT SUM(total_points) AS overall_total_points, contestant_id, SUM(percentage)/(SELECT COUNT(contest_id) FROM 
                                `contest` WHERE `event_id`='$event_id') AS overall_percentage, contestant_name, contestant_number 
                                  FROM `overall` 
                                  JOIN `contest` USING(contest_id) 
        ...
        $tabulatedResult = mysqli_query($connection, $tabulatedQuery);
                      ...
                      $total = 0;
                        while($rowTabulated = mysqli_fetch_assoc($tabulatedResult)){
                            if($total !=  $rowTabulated['overall_percentage']){
                                    $ranking++;
                                }
                                $total = $rowTabulated['overall_percentage'];  

?>

<?php if($ranking == 1){?>
    <center><span class="text-muted text-u" style="font-size: 20px;"><p class="text-warning"><b>Official Winner of <?php echo $rowEventName['event_name'] ?></b>&nbsp<span class="fa fa-check-circle"></span></p></span></center>
     <div class="padding">
      <div id="intro">
<br><br>
        <h5 class="_300 l-h">With total percentage of <strong><?php echo $rowTabulated['overall_percentage']?>%</strong> and total points of <strong><?php echo $rowTabulated['overall_total_points']?></strong>.</h5>
<br>
        <h4 class="300 l-h">Contestant #<b><?php echo $rowTabulated['contestant_number']?></b></h4>        
        <h2 class="m-b text-primary"><b><?php echo $rowTabulated['contestant_name']?></b></h2>
