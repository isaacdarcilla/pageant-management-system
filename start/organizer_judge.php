...
<div class="p-a white lt box-shadow">
  <div class="row">
    ...
    <div class="col-sm-6 text-sm-right">
      <div class="m-y-sm">
        <span class="m-r-sm">Manage Judges</span>
        <div class="btn-group dropdown">
              ...
              <div class="dropdown-menu dropdown-menu-scale pull-right">
                <a class="dropdown-item" href="organizer_add_judge.php?sponsor_id=<?php echo $sponsor_id ?>&username=<?php echo $username ?>&event_id=<?php echo $rowEventName['event_id']?>">Judges</a>
...
<div class="padding">
      <div class="container ">
          <h2 class="display-3 _700 l-s-n-3x m-t-lg m-b-md"><span class="text-primary">Judges Information</span></h2>
        <div class="row m-y-lg">
                       <?php 
                           $queryJudge = "SELECT *
                             FROM (SELECT * FROM judge_table WHERE registered_by_sponsor=$sponsor_id AND event_id=$event_id) AS result 
                               
                              LEFT JOIN (SELECT judge_id, event_id FROM judge_event WHERE added_by_sponsor = '$sponsor_id' AND event_id=$event_id) AS judges USING(judge_id);
                            ";
                                    
                                    $resJudge = mysqli_query($connection, $queryJudge);
                            while($rowJudges = mysqli_fetch_assoc($resJudge)){
                        ?>
          <div class="col-md-6 col-lg-4">
            <div class="box white box-shadow-z3 text-center" style="height: 200px">
                <a >
                <!--  <img class="img-responsive b-b m-b" src="../assets/image.jpg" alt="default"> --><br>
                  <span class="_800 p-a block h4 m-a-0"><span class="fa fa-star"></span>&nbsp <?php echo $rowJudges['first_name']?> 
                    <?php echo $rowJudges['sur_name']?> 
                  </span></a>
<p>
                  
                  Email <a href="mail-to:<?php echo $rowJudges['email']?>" class="text-success"><?php echo $rowJudges['email']?></a>
                </p>
                <div class="box-body">
                                    <?php
                                    if($rowJudges['event_id']==NULL)
                                        {
                  ...
                  <p>
                      ...
                      <a href="organizer_adding_judge.php?username=<?php echo $username?>&event_id=<?php echo $event_id ?>&sponsor_id=<?php echo $sponsor_id ?>&judge_id=<?php echo $rowJudges['judge_id'] ?>"><span class="btn btn-sm rounded primary text-u-c _700">
                  Select
                      </span></a>
                  </p>
                                    <?php 
                                        }else{
                                          $message_feed = '<center>'.$rowJudges['first_name'].' is already selected as judge for '.$rowEventName['event_name'].'. <span class=""><a class="text-white" href="organizer_event_details.php?username='.$username.'&sponsor_id='.$sponsor_id.'&event_id='.$event_id.'">
                <strong class="text-warning"><u>View profile</u></strong>.</a>
                      </span>  </center>';
                                          ?>

                                                                
                  <?php echo $message_feed ?> 
